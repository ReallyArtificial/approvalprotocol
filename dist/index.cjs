...
__export(index_exports, {
  ApActionSchema: () => ApActionSchema,
  ApConfirmParamsSchema: () => ApConfirmParamsSchema,
  ApContextSchema: () => ApContextSchema,
  ApDecideParamsSchema: () => ApDecideParamsSchema,
  ApNegotiateRequestSchema: () => ApNegotiateRequestSchema,
  ApPolicySchema: () => ApPolicySchema,
  ApRequestParamsSchema: () => ApRequestParamsSchema,
  ApRiskMetadataSchema: () => ApRiskMetadataSchema,
  ApRollbackParamsSchema: () => ApRollbackParamsSchema,
  ApUndoMetadataSchema: () => ApUndoMetadataSchema,
  ApprovalClient: () => ApprovalClient,
  ApprovalServer: () => ApprovalServer,
  ApprovalStore: () => ApprovalStore,
  BlastRadiusSchema: () => BlastRadiusSchema,
  CliChannel: () => CliChannel,
  DecisionSchema: () => DecisionSchema,
  DryRunResult: () => DryRunResult,
  EstimatedImpactSchema: () => EstimatedImpactSchema,
  ExecutionModeSchema: () => ExecutionModeSchema,
  PolicyRequiresSchema: () => PolicyRequiresSchema,
  UndoTypeSchema: () => UndoTypeSchema,
...
var BlastRadiusSchema = import_zod.z.enum([
  "self",
  "single_user",
  "team",
  "org",
...
var EstimatedImpactSchema = import_zod.z.enum([
  "financial",
  "data",
  "communication",
  "system"
]);
var ApRiskMetadataSchema = import_zod.z.object({
  reversible: import_zod.z.boolean(),
...
var ApActionSchema = import_zod.z.object({
  name: import_zod.z.string().min(1),
  description: import_zod.z.string().optional(),
  params: import_zod.z.record(import_zod.z.unknown())
});
var ApContextSchema = import_zod.z.object({
...
var PolicyRequiresSchema = import_zod.z.enum(["always", "never", "conditional"]);
var ExecutionModeSchema = import_zod.z.enum(["live", "sandbox", "dry_run"]);
var ApPolicySchema = import_zod.z.object({
  requires: PolicyRequiresSchema,
  when: import_zod.z.string().optional(),
  execution: ExecutionModeSchema.optional()
});
var UndoTypeSchema = import_zod.z.enum(["api_call", "function", "manual", "none"]);
var ApUndoMetadataSchema = import_zod.z.object({
  type: UndoTypeSchema,
  description: import_zod.z.string().optional(),
  instructions: import_zod.z.record(import_zod.z.unknown()).optional()
});
var ApNegotiateRequestSchema = import_zod.z.object({
  agent: import_zod.z.string().min(1),
  capabilities: import_zod.z.array(import_zod.z.string().min(1)).min(1)
});
var ApRequestParamsSchema = import_zod.z.object({
  ...
  callback_url: import_zod.z.string().url().optional()
});
var DecisionSchema = import_zod.z.enum(["approve", "deny", "edit"]);
var ApDecideParamsSchema = import_zod.z.object({
  request_id: import_zod.z.string().min(1),
  decision: DecisionSchema,
  modified_params: import_zod.z.record(import_zod.z.unknown()).optional(),
  reason: import_zod.z.string().optional(),
  decided_by: import_zod.z.string().min(1)
});
var ApConfirmParamsSchema = import_zod.z.object({
  request_id: import_zod.z.string().min(1),
  success: import_zod.z.boolean(),
  result: import_zod.z.record(import_zod.z.unknown()).optional(),
  undo: ApUndoMetadataSchema.optional(),
  error: import_zod.z.string().optional(),
  confirmed_by: import_zod.z.string().min(1)
});
var ApRollbackParamsSchema = import_zod.z.object({
  request_id: import_zod.z.string().min(1),
  reason: import_zod.z.string().optional(),
  initiated_by: import_zod.z.string().min(1)
});
